...
public class MemberController {
	...
	public String kakaoLoginGet(String nickName, String email, String accessToken, HttpServletRequest request,
		...
		// 카카오 로그인한 회원인 경우에는 우리 회원인지를 조사한다.. (넘어온 이메일을 @기준으로 아이디와 분리해서 기존 member 테이블의 아이디와 비교한다.)
		...
		if(vo == null) {
			// 아이디 결정하기, 카카오로그인시 카카오에 등록된 이메일의 @ 기준으로 잘라 앞의 부분을 아이디로 저장하여 넘긴다.
	...
	public String kakaoLogoutGet(HttpSession session) {
		...
		try {
			...
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();	//add upcasting
			conn.setRequestMethod("POST");
			conn.setRequestProperty("Authorization", " " + accessToken);	//Authorization 예약어, " " 공백있어야함
			
			// 카카오에서 정상적으로 처리 되었다면 200번이 들어온다.
