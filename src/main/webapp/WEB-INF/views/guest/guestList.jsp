...
<div class="container">
  ...
  <table class="table table-borderless m-0 p-0">
	...
	<c:set var="curScrStartNo" value="${curScrStartNo}"></c:set>  <!-- 인덱스 번호 중간에 삭제해도 빵꾸 안나게 번호 처리 -->
...
<%-- 				<c:if test="${vo.email != '' && vo.email != null}">${vo.email}</c:if>       위랑 이거는 한짝--%> 
	...
	<div class="text-center">
		...
		<c:if test="${pag > 1 }"><li class="page-item"><a class="page-link text-secondary" href="guestList?pag=1&pageSize=${pageSize}">첫 페이지</a></li></c:if>
		...
		<c:forEach var="i" begin="${(curBlock * blockSize)+1}" end="${(curBlock * blockSize) + blockSize}" varStatus="st">
			<!-- 현재있는 페이지 수만 굵게 표시한다. -->
			<c:if test="${i <= totPage && i == pag}"><li class="page-item active"><a class="page-link" href="guestList?pag=${i}&pageSize=${pageSize}">${i}</a></li></c:if> 
		...
		<c:if test="${pag < totPage}"><li class="page-item"><a class="page-link text-secondary" href="guestList?pag=${totPage}&pageSize=${pageSize}">마지막 페이지</a></li></c:if>
		</ul>
	</div>
	<!-- 블록페이지 끝 -->
</div>
