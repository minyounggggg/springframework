...
<head>
	...
	<script>
		...
		function fCheck() {
			...
			else if(ext != "jpg" && ext != "gif" && ext != "png" && ext != "zip" && ext != "ppt" && ext != "pptx"){
				alert("업로드 가능한 파일은 jpg, gif, png, zip, ppt, pptx입니다.")
			}
			else {
				myform.submit();
			}
...
<div class="container">
	...
	<div id="downLoadFile">
		...
		<div class="row">
			...
			<div class="col">
				<input type="button" value="폴더내 모든 파일 삭제" onclick="fileDeleteAll()" class="btn btn-danger"/>
			</div>
		</div>
			...
			<c:forEach var="file" items="${files}" varStatus="st">
				<tr>
					...
					<td>
						...
						<c:set var="extName" value="${fn:toLowerCase(fNameArray[fn:length(fNameArray)-1])}" />	<!-- .을 기준을 확장자를 알기위해 -->
